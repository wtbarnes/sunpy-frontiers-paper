...
Solar time series data, however, do not have a standard metadata or file format and may be stored in a variety of file formats, including FITS, netCDF, JSON, or plain text.}
\added[id=NF]{As such, the metadata associated with each \code{TimeSeries} object is much more sparse compared to \code{Map}, but at minimum will include the time of each observation as well as the associated instrument that made the observation.}
