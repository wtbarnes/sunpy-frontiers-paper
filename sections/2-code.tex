...
\added[id=NF]{\sunpypkg provides detailed Application Programming Interface (API) documentation}\footnote{https://docs.sunpy.org/en/stable/}.
...
\added[id=WTB]{Whereas, Section 4.1.1 of} \citep{sunpy_community2020} \added[id=WTB]{also provides a comprehensive discussion of the data sources that \Fido searches by default.}
...
\added[id=WTB]{\sunpypkg includes thousands of unit, regression and integration tests that are run using the \code{pytest} testing framework and includes tests of the documentation.
...
\added[id=NF]{The goal of the \sunpyproj is to slowly increase the number of affiliated packages.
This brings the challenge of ensuring that the quality of all affiliated packages never declines.
...
In the future (funded by a Roses Grant), automation of these metrics and collating more information on the \sunpyproj website is planned.
In addition, we would like to start doing annual re-reviews of all affiliated packages.
...
\added[id=WTB]{We have included a Jupyter notebook that illustrates each step of this workflow in the \github repository that accompanies this paper.}\footnote{The \github repository for this paper, including the complete text and all code to generate Figure \autoref{fig:affiliated-package-showcase}, can be found at \url{https://github.com/sunpy/sunpy-frontiers-paper}}
