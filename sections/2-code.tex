...
\added[id=NF]{information on the Application Programming Interface for \sunpypkg is provided by the documentation}\footnote{https://docs.sunpy.org/en/stable/}.
...
\added[id=WTB]{Section 4.1.1 of} \citep{sunpy_community2020} \added[id=WTB]{also provides a more detailed discussion of the data sources that \Fido searches by default.}
...
Once a user has downloaded data, the \code{TimeSeries} and \code{Map} data objects can be used to load time series and two-dimensional image data, respectively.
...
\added[id=WTB]{\sunpypkg includes thousands of unit and integration tests that are run using the \code{pytest} testing framework as well as \code{tox} for ensuring tests are run in isolated environments.
This test suite also includes tests of the documentation.
This test suite is run on every pull request opened on the \sunpypkg \github using \github Actions to ensure that contributions to the codebase do not lead to unexpected regressions.
Additionally, we also make use of the Circle CI platform.
...
\added[id=WTB]{By providing regularly scheduled-scheduled, versioned releases of \sunpypkg, the \sunpyproj enables reproducibility.
...
\added[id=WTB]{This release process is completely automated through GitHub Actions.}\footnote{The \github Actions templates used by \sunpypkg and the affiliated packages within the \sunpyproj are available here: \url{https://github.com/OpenAstronomy/github-actions-workflows}}
...
If the packages build successfully, they are automatically uploaded to to Python Package Index (PyPi).
Subsequently, a PR is automatically opened on the \code{conda-forge} feedstock to upload the release to \code{conda-forge}.}
...
\added[id=NF]{As the number of \sunpyproj affiliated packages increases, it is important to consider how to ensure that the quality of the affiliated packages does not change.
Quality is somewhat subjective when it comes to software, as the Application process below outlines, several metrics enable a cursory glance at the quality of a package.
In the future (due to the Roses Grant), work will undertaken to automate the collection of these metrics and report them back to the wider community and we would like to start doing annual re-reviews of all affiliated packages.
There is much work to be done on this front and we welcome any contributions to this effort.}
...
\added[id=NF]{Interoperability, in this context, means that affiliated packages should make use of the existing \sunpypkg core data structures, (e.g., \code{Map} and \code{Timeseries}), wherever possible in lieu of their own custom data structures.
...
\added[id=WTB]{We have included a Jupyter notebook that illustrates each step of this workflow in the GitHub repository that accompanies this paper.}\footnote{The GitHub repository for this paper, including the complete text and all code to generate Figure 2, can be found at \url{https://github.com/sunpy/sunpy-frontiers-paper}}
